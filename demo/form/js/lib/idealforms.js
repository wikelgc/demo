...
var Utils = {
  ...
  makeInput: function( name, value, type, list, placeholder ) {
    ...
    // Text & file
    if ( /^(text||email|number|search|url|tel|file|hidden)$/.test(type) )
      markup = '<input '+
        'type="'+ type +'" '+
        'id="'+ name +'" '+
        'name="'+ name +'" '+
  ...
  var filters = {
    ...
    ajax: {
      regex: function( input, value, showOrHideError ) {
        ...
        var userAjaxOps = input.userOptions.data;
    ...
    inSet:{
    	regex:function(input,value){
    		var regex = input.userOptions.data.regex;
    		if(regex){
    			return eval("/"+regex+"/").test(value);
    		}
    		return true;
    	}
    },
    condition:{
    	regex:function(input,value){
    		var targetName = input.userOptions.data.targetName;
    		var targetValue = input.userOptions.data.targetValue;
    		var include = input.userOptions.data.include;
    		var exclude = input.userOptions.data.exclude;
    		var $input = input.input;
    		var $form = $input.parents().filter("form");
    		var $target =$form.find("*[name="+targetName+"]");
    		if($target.val() == targetValue){
    			if(include && !value.test(eval("/"+include+"/"))){
    				return false;
    			}
    			
    			if(exclude && value.test(eval("/"+exclude+"/"))){
    				return false;
    			}
    		}
    		return true;
